...
%DIF DEL main_org.tex   Fri Jul 18 14:03:25 2025
%DIF ADD main_rev.tex   Mon Jul 28 22:24:07 2025
% https://who-umc.org/research/data-driven-discovery/ %DIF > 
%DIF -------
...
\usepackage{natbib}
%DIF 60a61-62
\usepackage[normalem]{ulem} % For strikethrough text %DIF > 
\usepackage{fontawesome5} %DIF > 
%DIF -------
...
\definecolor{BLUE}{rgb}{0,0,1} % Define BLUE as an alias for standard blue %DIF > 
\definecolor{GREEN}{rgb}{0,1,0} % Define BLUE as an alias for standard blue %DIF > 
\definecolor{RED}{rgb}{1,0,0} % Define BLUE as an alias for standard blue %DIF > 
 %DIF > 
% \newcommand{\removed}[1]{\textcolor{red}{#1}} %DIF > 
\newcommand{\removed}[1]{\textcolor{red}{\sout{#1}}} %DIF > 
\newcommand{\added}[1]{\textcolor{blue}{#1}} %DIF > 
\newcommand{\changed}[1]{\textcolor{green}{#1}} %DIF > 
 %DIF > 
%DIF -------
...
% }
\title{\DIFdelbegin \DIFdel{Framework for }\DIFdelend Implementation of \DIFdelbegin \DIFdel{Poisson MaxSPRT Technique with Variations }\DIFdelend \DIFaddbegin \DIFadd{a framework to apply Sequential Analysis }\DIFaddend for Vaccine Safety \DIFaddbegin \DIFadd{Surveillance on SAEFVIC Dataset}\DIFaddend }
...
\DIFadd{Timely detection of rare adverse events following immunisation (AEFI) is critical for }\DIFaddend vaccine safety\DIFaddbegin \DIFadd{. For post-market }\DIFaddend surveillance, \DIFdelbegin \DIFdel{the Maximized }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis techniques that were initially created for clinical trials~\mbox{%DIFAUXCMD
\citep{gordon1983discrete} }\hskip0pt%DIFAUXCMD
have been modified to allow for near real-time signal detection while strictly controlling Type I~\mbox{%DIFAUXCMD
\citep{silva2018type} }\hskip0pt%DIFAUXCMD
and Type II errors.  The conventional }\DIFaddend Sequential Probability Ratio Test (\DIFdelbegin \DIFdel{MaxSPRT}\DIFdelend \DIFaddbegin \DIFadd{SPRT}\DIFaddend )\DIFdelbegin \DIFdel{was developed }\DIFdelend \DIFaddbegin \DIFadd{, however fundamental, has limitations due to its dependence on predetermined alternatives and sensitivity to predefined relative risks.  These drawbacks are addressed }\DIFaddend by the \DIFdelbegin \DIFdel{Centers for Disease Control and Prevention }\DIFdelend \DIFaddbegin \DIFadd{Maximised SPRT }\DIFaddend (\DIFdelbegin \DIFdel{CDC}\DIFdelend \DIFaddbegin \DIFadd{MaxSPRT}\DIFaddend )\DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{kulldorff2011maximized}}\hskip0pt%DIFAUXCMD
. Its predictive value and power to detect signals }\DIFdelend \DIFaddbegin \DIFadd{, which supports both Poisson }\DIFaddend and \DIFdelbegin \DIFdel{the ability to monitor adverse events continuously have made it an emerging technique }\DIFdelend \DIFaddbegin \DIFadd{Binomial models }\DIFaddend for \DIFdelbegin \DIFdel{vaccine adverse event surveillance. Moreover, being able to use }\DIFdelend a \DIFdelbegin \DIFdel{statistical method }\DIFdelend \DIFaddbegin \DIFadd{variety of surveillance designs and permits composite alternatives (}\DIFaddend e.g.\DIFdelbegin \DIFdel{MaxSPRT in the absence of dose distributed denominator is a practical advantage for spontaneous reporting systems to function as stand-alone signal detection systems. In this paper}\DIFdelend , \DIFdelbegin \DIFdel{we present a comprehensive framework for implementing MaxSPRT for vaccine safety surveillance and Poisson data}\DIFdelend \DIFaddbegin \DIFadd{$RR > 1$)}\DIFaddend .
\DIFdelbegin \DIFdel{We analysed }\DIFdelend \DIFaddbegin \DIFadd{By conditioning on observed events, }\DIFaddend the \DIFdelbegin \DIFdel{literature regarding }\DIFdelend \DIFaddbegin \DIFadd{Conditional }\DIFaddend MaxSPRT \DIFaddbegin \DIFadd{(CMaxSPRT) goes one step further }\DIFaddend and \DIFdelbegin \DIFdel{sequential analysis. Our analysis revealed numerous variations }\DIFdelend \DIFaddbegin \DIFadd{maintains error validity even when there is a lack }\DIFaddend of \DIFdelbegin \DIFdel{MaxSPRT}\DIFdelend \DIFaddbegin \DIFadd{historical data.  Alpha spending functions}\DIFaddend , \DIFdelbegin \DIFdel{adapted }\DIFdelend \DIFaddbegin \DIFadd{especially when optimised~\mbox{%DIFAUXCMD
\citep{silva2020optimal} }\hskip0pt%DIFAUXCMD
by linear programming techniques, provide better control over error rates }\DIFaddend to \DIFdelbegin \DIFdel{the specific requirements }\DIFdelend \DIFaddbegin \DIFadd{handle repeated testing, allowing for more effective designs with lower expected time to signal (ETS) }\DIFaddend and \DIFdelbegin \DIFdel{objectives of the users}\DIFdelend \DIFaddbegin \DIFadd{expected sample size (ESS)}\DIFaddend .
\DIFdelbegin \DIFdel{Variations }\DIFdelend \DIFaddbegin 

\DIFadd{Despite these methodological breakthroughs, spontaneous surveillance systems }\DIFaddend are \DIFdelbegin \DIFdel{due }\DIFdelend \DIFaddbegin \DIFadd{still difficult }\DIFaddend to \DIFdelbegin \DIFdel{differing types of data }\DIFdelend \DIFaddbegin \DIFadd{apply in the real world }\DIFaddend and \DIFdelbegin \DIFdel{purpose of use, including whether used for epidemiological surveillance or for regulatory monitoring}\DIFdelend \DIFaddbegin \DIFadd{lack standardisation}\DIFaddend .  This paper \DIFdelbegin \DIFdel{provides }\DIFdelend \DIFaddbegin \DIFadd{offers }\DIFaddend a \DIFdelbegin \DIFdel{comprehensive guide for organisations contemplating the implementation of MaxSPRT}\DIFdelend \DIFaddbegin \DIFadd{thorough approach to using sequential analysis in real-world settings, encompassing everything from data wrangling to signal detection}\DIFaddend .  \DIFdelbegin \DIFdel{It synthesises }\DIFdelend \DIFaddbegin \DIFadd{By synthesising }\DIFaddend existing literature\DIFdelbegin \DIFdel{on MaxSPRT}\DIFdelend , \DIFdelbegin \DIFdel{identifies variations based on specific requirements}\DIFdelend \DIFaddbegin \DIFadd{we identify crucial contextual factors that influence design choices, such as data structure, surveillance objectives}\DIFaddend , and \DIFdelbegin \DIFdel{describes an implementation framework}\DIFdelend \DIFaddbegin \DIFadd{vaccine-AEFI characteristics}\DIFaddend .  \DIFdelbegin \DIFdel{We offer a detailed explanation of the steps and challenges associated with the implementation of MaxSPRT on the adverse event following immunisation (AEFI)  reporting database of Surveillance of Adverse Events Following Vaccination in the Community}\DIFdelend \DIFaddbegin \DIFadd{With SAEFVIC}\DIFaddend , Victoria\DIFaddbegin \DIFadd{'s jurisdictional vaccine safety system}\DIFaddend , \DIFdelbegin \DIFdel{Australia (SAEFVIC)}\DIFdelend \DIFaddbegin \DIFadd{as a case study, we outline implementation steps, talk about common issues}\DIFaddend , \DIFdelbegin \DIFdel{the largest jurisdictional reporting service by volume in Australia. It also proposes some techniques }\DIFdelend and \DIFdelbegin \DIFdel{measures }\DIFdelend \DIFaddbegin \DIFadd{provide solutions.  This work acts as a guide for organisations intending }\DIFaddend to \DIFdelbegin \DIFdel{deal with the challenges associated with the implementation process}\DIFdelend \DIFaddbegin \DIFadd{start from scratch when integrating sequential analysis into vaccine pharmacovigilance operations}\DIFaddend .
} 

\DIFdelbegin %DIFDELCMD < \keywords{Continuous Surveillance, Adverse events, Vaccine pharmacovigilance, Sequential Probability Ratio, Signal detection, reporting delay, real-world datasets}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \keywords{Sequential Analysis, Continuous and Group Surveillance, Adverse events, Vaccine pharmacovigilance, Sequential Probability Ratio, Signal detection, reporting delay, real-world datasets}
\DIFaddend 

\section*{Key Points}
\DIFaddbegin \DIFadd{Sequential analysis especially }\DIFaddend MaxSPRT is a powerful method for ongoing vaccine surveillance, offering flexibility to adapt to various situations and data limitations. However, \DIFdelbegin \DIFdel{this flexibility }\DIFdelend \DIFaddbegin \DIFadd{the offered flexibility and variations as well as the lack of step by step guidance }\DIFaddend can lead to challenges in implementation. Our paper simplifies the MaxSPRT method with clear explanations and step-by-step guidance \DIFaddbegin \DIFadd{from scratch with illustrative computation}\DIFaddend , addressing potential issues and proposing solutions to improve its use in monitoring vaccine and drug safety. \DIFaddbegin \DIFadd{Furthermore, an extendable and generic tool to implement such analysis from scratch that includes data wrangling and preprocessing.
}\DIFaddend 
...
%DIF >  \removed{Consequently, Kulldorff et al. proposed the use of a maximised sequential probability ratio test (MaxSPRT)} \citep{kulldorff2011maximized} \removed{based on a composite alternative hypothesis, which works across a range of relative risks. However, the various, specific objectives and data sources of vaccine surveillance groups have added to the complexity of implementing them using the different measures and parameters required for MaxSPRT analysis.}
\DIFaddbegin 

%DIF >  \removed{To our knowledge, there is currently no literature describing the variations of MaxSPRT implementation, detailed and step-wise calculations, along with when to use them based on vaccine surveillance objectives, and data sources and types.} 

%DIF >  \added{ADD A PARAGRAPH REVISE THIS ONE SO THAT IT IS KIND OF A REVIEW OF HOW MAXSPRT, B,C AND P MAXSPRT CAME FROM SPRT AND HOW GROUP AND CONTINUOUS CAME INTO BEING. HOW ALPHA SPENDING PLAN ALSO HELPS}
%DIF >  \added{May be some other motivations ... needed behind this paper ...}

\DIFadd{Sequential analysis for post-market surveillance evolved to enable early detection of rare adverse events while controlling error rates. }\DIFaddend An important aspect of vaccine surveillance is \DIFaddbegin \DIFadd{the }\DIFaddend continuous monitoring of adverse events that vaccinees may experience \DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{when }\DIFaddend vaccines are administered, through a continuous sequential analysis that detects a signal when adverse events exceed the probability that they occur by chance \citep{kulldorff2011maximized}. Sequential Probability Ratio Test (SPRT) \citep{wald1992sequential} was proposed by Wald to detect adverse events through hypothesis testing. It \DIFdelbegin \DIFdel{is used to determine the most likely hypothesis among a set of hypotheses }\DIFdelend \DIFaddbegin \DIFadd{assumes fixed alternatives and known expected event rates }\DIFaddend and is appropriate for sequentially independent and identically distributed (iid) data, which implies that the data comes from the same distribution. SPRT \DIFdelbegin \DIFdel{was then refined for vaccine surveillance, and we refer to this as Classical SPRT (cSPRT) in this paper. cSPRT }\DIFdelend is used for surveillance to test if the null hypothesis (no vaccine safety signal or the vaccine is safe), is more likely than the alternative hypothesis (there is a vaccine safety signal). However, \DIFdelbegin \DIFdel{cSPRT }\DIFdelend \DIFaddbegin \DIFadd{SPRT }\DIFaddend is highly sensitive to the relative risk chosen in the alternative hypothesis construction, which makes it difficult to use for ongoing, routine surveillance.

\DIFdelbegin \DIFdel{Consequently, Kulldorff et al. proposed the use of a maximised sequential probability ratio test }\DIFdelend \DIFaddbegin \DIFadd{To overcome SPRT’s rigidity and sensitivity, the Maximized SPRT }\DIFaddend (MaxSPRT) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{kulldorff2011maximized} }\hskip0pt%DIFAUXCMD
based on }\DIFdelend \DIFaddbegin \DIFadd{was introduced, allowing for }\DIFaddend a composite alternative \DIFdelbegin \DIFdel{hypothesis, which works across a range of relative risks.
However, the various, specific objectives and data sources of vaccine surveillance groups have added to the complexity of implementing them using the different measures and parameters required for MaxSPRT analysis. This subsequently resulted in various implementations of MaxSPRT by other researchers in the community}\DIFdelend \DIFaddbegin \DIFadd{(e.g., $RR>1$) and implemented for both Poisson and Binomial models. MaxSPRT facilitated real-time safety monitoring by enabling signals to occur even after a single event, depending on its timing. Poisson MaxSPRT is best for rate-based monitoring having time-to-event data or counts over exposure, while Binomial MaxSPRT fits self-controlled or matched cohort designs where outcomes are binary (event or no event).
}

\DIFadd{However, when expected event rates are estimated from limited historical data, MaxSPRT can inflate Type I error. To adjust for such issues, Li and Kulldorff proposed the Conditional MaxSPRT (CMaxSPRT), which incorporates variability in both historical and surveillance data by conditioning on observed events, thus preserving Type I error validity under small-sample conditions or when historical data is sparse.
}

\DIFadd{To further improve control over false positives across multiple looks at data, alpha spending functions were introduced. These functions (e.g., power-type, Pocock~\mbox{%DIFAUXCMD
\citep{pocock1977group}}\hskip0pt%DIFAUXCMD
, O’Brien-Fleming~\mbox{%DIFAUXCMD
\citep{o1979multiple}}\hskip0pt%DIFAUXCMD
) spread the allowable Type I error across time, with concave forms ($\rho<1$) minimizing expected time to signal (ETS) and convex ones ($\rho>1$) minimizing expected sample size (ESS). Exact optimization of alpha spending using linear programming (Silva et al., 2020) enables designs that minimize ETS while controlling Type I and Type II errors, and sample sizes, and maximizing statistical power, outperforming classical methods. Note that it is evident from \mbox{%DIFAUXCMD
\citep{r2021exact} }\hskip0pt%DIFAUXCMD
that alpha spending is implicitly defined by sequential test plans, regardless of the test statistic scale that is selected. This can be readily illustrated with the R Sequential~\mbox{%DIFAUXCMD
\citep{rSequential} }\hskip0pt%DIFAUXCMD
package. However, since identical alpha spending produces equivalent results across all scales, the practical lesson is not the test statistic selected but rather the careful selection of a near-optimal alpha spending plan. The package integrates Silva and Kulldorff's~\mbox{%DIFAUXCMD
\citep{silva2015continuous} }\hskip0pt%DIFAUXCMD
methodological advancements. A significant improvement is the replacement of the linear interpolation method suggested by Kulldorff et al.~\mbox{%DIFAUXCMD
\citep{kulldorff2011maximized} }\hskip0pt%DIFAUXCMD
with a bisection procedure to precisely compute exact critical values (CVs). This method may result in infinite loops depending on the fixed expected number of events. Moreover, Silva and Kulldorff~\mbox{%DIFAUXCMD
\citep{silva2015continuous} }\hskip0pt%DIFAUXCMD
showed that, in contrast to depending on the continuous critical values that ~\mbox{%DIFAUXCMD
\citep{kulldorff2011maximized} }\hskip0pt%DIFAUXCMD
had previously tabulated, employing group sequential tests with unpredictable sample sizes through alpha spending functions greatly improves statistical power}\DIFaddend .

\DIFdelbegin \DIFdel{To our knowledge, there is currently no literature describing the variations of MaxSPRT implementation , detailed and step-wise calculations, along with when to use them based on vaccine surveillance objectives, }\DIFdelend \DIFaddbegin \DIFadd{The R Sequential package~\mbox{%DIFAUXCMD
\citep{rSequential} }\hskip0pt%DIFAUXCMD
is central to the surveillance system developed within SAEFVIC; all data analyses, experimental results, visualizations, and related research presented in this paper were conducted using this package.
}

\DIFadd{Silva et al.'s extensive guide~\mbox{%DIFAUXCMD
\citep{r2021exact}  }\hskip0pt%DIFAUXCMD
on planning sample sizes and the alpha spending approach in sequential analysis is indeed critical for maintaining the desired statistical power. Their tutorial presents detailed methodological insights and introduces optimized alpha spending plans to minimize the expected time to signal (ETS) and expected sample size (ESS). However, while Silva et al.'s work expertly addresses statistical optimizations, our paper takes a more practical, step-by-step approach tailored for real-world implementation, particularly highlighting common challenges such as data wrangling and preprocessing that arise in operational vaccine safety surveillance systems like SAEFVIC..
}

\DIFadd{Moreover, Leite et al.~\mbox{%DIFAUXCMD
\citep{leite2016near} }\hskip0pt%DIFAUXCMD
present a comprehensive systematic review of real-world applications of statistical methods in vaccine safety surveillance, presenting a variety of statistical tools and approaches. Their work outlines practical surveillance methods using electronic health records and provides foundational insights into the surveillance landscape. In contrast, our manuscript uniquely integrates these surveillance methods into a hands-on, comprehensive framework, clearly detailing implementation steps, parameter selection, }\DIFaddend and \DIFdelbegin \DIFdel{data sources and types.
}\DIFdelend \DIFaddbegin \DIFadd{strategies for addressing common real-world issues like reporting delays and missing data.
}

\DIFadd{Our guide addresses important practical gaps not fully explored in the literature, including:
}\begin{itemize*}
    \item \DIFadd{Explicit procedures for data preprocessing and handling of missing values and noisy data.
    }\item \DIFadd{Detailed computation and interpretation of real-time measures such as the observed exposed person-time ($L_{stiwa}$) and the proportion of data completeness ($p(s, t)$), essential for accurate real-time signal detection.
    }\item \DIFadd{Flexible options for adapting to various surveillance scenarios, such as age stratification and dynamic binning, not extensively detailed in existing literature.
    }\item \DIFadd{Clear illustration of calculating the expected number of events, addressing and calculating reporting delays and proportion of data complete, leveraging practical heuristics and real-world constraints, thereby providing explicit operational guidance beyond theoretical formulations.
}\end{itemize*}

\DIFadd{By explicitly addressing these pragmatic considerations and providing a comprehensive, illustrative approach to sequential analysis for vaccine safety, our manuscript offers complementary value to existing tutorials and reviews. Thus, our guide serves not as a replacement but rather as an essential operational extension to the seminal methodological contributions of Silva et al.~\mbox{%DIFAUXCMD
\citep{r2021exact} }\hskip0pt%DIFAUXCMD
and the foundational review by Leite et al.~\mbox{%DIFAUXCMD
\cite{leite2016near}}\hskip0pt%DIFAUXCMD
.
}

%DIF >  It began with Wald’s Sequential Probability Ratio Test (SPRT), which tests hypotheses continuously but assumes a fixed alternative and known expected event rates.
%DIF >  Key metrics—ETS (average time to signal, conditional on rejection), ESS (overall average sample size), power, and error rates—guide trade-offs. Innovations like delayed starts, minimum-event thresholds, and conditional modeling collectively enhance timely, efficient, and statistically sound safety surveillance.
%DIF >  SPRT was then refined for vaccine surveillance.
\DIFaddend 

\section{Methods}\label{sec:methods}

\DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{In addition to the introductory developmental trajectory provided in Section~\ref{sec:introduction}, this }\DIFaddend section describes the evolution of \DIFaddbegin \DIFadd{sequential analysis focused on likelihood ratio tests such as }\DIFaddend SPRT and MaxSPRT as reported in the literature. It forms the foundation for the framework proposed in this paper for the implementation of MaxSPRT technique variations for vaccine safety. \DIFaddbegin \DIFadd{We look at how variations in data types, distribution features, parameter availability, and organisational constraints—such as concerns about data timeliness and completeness—all affect the form and application of sequential analysis methods, leading to variations in implementation strategies and surveillance results.
}\DIFaddend 
...
\begin{figure*}[!htb]
    ...
    \caption{\DIFdelbeginFL \DIFdelFL{Different variations of MaxSPRT}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Sequential Analysis: Comparative View Across Parameters and Conditions.}\DIFaddendFL }
    \label{fig:variationMaxSPRT}
\end{figure*}

\DIFaddbegin \DIFadd{We start with the difference in the calculation of the parameters for SPRT and MaxSPRT. }\DIFaddend In both Wald’s SPRT\DIFdelbegin \DIFdel{and cSPRT }\DIFdelend \DIFaddbegin \DIFadd{~}\DIFaddend \citep{wald1992sequential}, and Kulldorff's MaxSPRT \citep{kulldorff2011maximized}, the number of adverse events is considered to be random, while the cumulative person-time or the cumulative number of vaccinations, if the follow-up time per vaccination is uniform, is considered to be fixed. The predicted number of adverse events under the null hypothesis is then considered to be a known function of the total person-time and certain possible confounding variables like age, sex, and location. \DIFaddbegin \DIFadd{Figure~\ref{fig:variationMaxSPRT} denotes this variation by the first branching that occurs for sequential analysis between SPRT and MaxSPRT. Note, another factor that signifies the difference is, SPRT is highly sensitive to the relative risk chosen in the alternative hypothesis and cannot deal with composite alternatives.
}\DIFaddend 

Based on the \DIFaddbegin \DIFadd{mode of analysis—whether cohort-based or case-based—MaxSPRT can be applied in two primary forms: Poisson MaxSPRT for cohort-based analyses and Binomial MaxSPRT for case-based or self-controlled designs. The Binomial MaxSPRT (bMaxSPRT) is suitable for self-controlled approaches in which the comparator is calculated from contemporary rather than historical data and the analysis compares the presence or absence of an event (binary outcome). 
}

\DIFadd{Furthermore, the }\DIFaddend dependence and sufficiency of the comparator data \DIFdelbegin \DIFdel{MaxSPRT can be divided into two groups, }\DIFdelend \DIFaddbegin \DIFadd{lead to another branching: }\DIFaddend (i) Conditional MaxSPRT \DIFaddbegin \DIFadd{(cMaxSPRT) }\DIFaddend and (ii) unconditional\DIFdelbegin \DIFdel{MaxSPRT}\DIFdelend \DIFaddbegin \DIFadd{/conventional MaxSPRT (we refer to it as pMaxSPRT for te sake of simplicity)}\DIFaddend . When the background information is insufficient and conditional on an event, \DIFdelbegin \DIFdel{Kulldorff }\DIFdelend \DIFaddbegin \DIFadd{Li et al. }\DIFaddend developed conditional MaxSPRT (cMaxSPRT)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{FDAMaxSPRT2021}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{li2010conditional}}\hskip0pt%DIFAUXCMD
}\DIFaddend , where it factors in the number of adverse events in the historical data and the surveillance population and computes the cumulative person-time it took to observe so many events as the random variable.

\DIFdelbegin \DIFdel{The unconditional MaxSPRT can be further subdivided based on the properties of the dataset into two types~\mbox{%DIFAUXCMD
\citep{kulldorff2011maximized,FDAMaxSPRT2021,li2010conditional}}\hskip0pt%DIFAUXCMD
: (i) pMaxSPRT for Poisson distribution data where values are real numbers, comparator data are not conditional on any event and sufficient background information is available; and 
    (ii) bMaxSPRT for data with a binomial distribution where the SPRT analysis is conducted for each event being observed that is binary for each event}\DIFdelend \DIFaddbegin \DIFadd{In addition to these methodological variations, Silva et al.’s introduction of optimised alpha spending plans adds a further dimension to MaxSPRT design. By distributing the Type I error across interim analyses using tailored spending functions, this approach enhances control over both Type I and Type II errors, increases statistical power and significance, and effectively reduces the expected time to signal ($ETS$) by allowing early stopping, while also optimising the expected sample size ($ESS$) to increase statistical power. This optimisation, often achieved through linear programming, enables more efficient and robust sequential surveillance compared to traditional fixed-threshold designs. Here, $ETS$, $ESS$  and $CV$ are calculated dynamically based on the choice of alpha spending functions. There are generally three categories of alpha expenditure functions: }\begin{enumerate*}[label={(\roman*)}]
    \item \DIFadd{Linear,
    }\item \DIFadd{Convex, and 
    }\item \DIFadd{Concave.
}\end{enumerate*}
\DIFadd{Silva et al. further demonstrated that the analytical objective should guide the choice of alpha spending function among the three types. Concave functions, which use a larger portion of the Type I error budget at the start, are preferable for early signal detection. Although this lowers ETS, it comes at the cost of reduced statistical power. Convex expenditure functions, on the other hand, preserve statistical power in longer surveillance windows by conserving alpha for subsequent analyses, providing improved control of ESS and more stable estimation when previous data is ambiguous}\DIFaddend .


%DIF >  \removed{The unconditional MaxSPRT can be further subdivided based on the properties of the dataset into two types} ~\citep{kulldorff2011maximized,FDAMaxSPRT2021,li2010conditional} \removed{: (i) pMaxSPRT for Poisson distribution data where values are real numbers, comparator data are not conditional on any event and sufficient background information is available; and (ii) bMaxSPRT for data with a binomial distribution where the SPRT analysis is conducted for each event being observed that is binary for each event.} \added{TOTALLY WRONG....}
\DIFaddbegin 

%DIF >  These make the calculation of MaxSPRT quite challenging due to numerous variations.

\DIFaddend The trend of variations continues for some further factors and parameters. \DIFdelbegin \DIFdel{These make the calculation of MaxSPRT quite challenging due to numerous variations. }\DIFdelend The parameters, hyperparameters, and conditions that need to be considered while \DIFdelbegin \DIFdel{calculating MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{performing sequential analysis }\DIFaddend are: 
\begin{enumerate*}[label={(\roman*)}]
    \item the grouping of the reports based on age;
    \item the decision to use any comparator rate, sources of \DIFaddbegin \DIFadd{the }\DIFaddend comparator rate, the process of calculating the comparator rate;
    \item the length of the risk window and the decision to slice or not slice risk windows;
    \item \DIFaddbegin \DIFadd{using self-controls or other controls;
    }\item \DIFadd{choosing the unvaccinated population in the concurrent data as comparator;
    }\item \DIFaddend the decision to use the length of exposure \DIFdelbegin \DIFdel{, written as ``$l_{stiwa}$" in the figure (Figure~\ref{fig:variationMaxSPRT}) and later denoted as }\DIFdelend \DIFaddbegin \DIFadd{and the method to compute it, e.g., using }\DIFaddend $l_{stiwa}$\DIFdelbegin \DIFdel{in the rest of the paper}\DIFdelend , as a proxy \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the calculation of exposed person time, and the variations in calculating the exposed person-time;
    \item the choice of using a test margin\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{kulldorff2011maximized} }\hskip0pt%DIFAUXCMD
}\DIFaddend in the calculation; and
    \item the decision to use a delay adjustment, as well as the ways to calculate the delay adjustment.
\end{enumerate*}

Each combination of the aforementioned factors makes a different variation of \DIFdelbegin \DIFdel{MaxSPRT}\DIFdelend \DIFaddbegin \DIFadd{sequential analysis}\DIFaddend , therefore, the factors and hyperparameters are introduced briefly as follows and described in greater detail with relevant technical information below.

\begin{itemize}
    \item \textbf{Age grouping:} Sometimes, we can stratify the whole MaxSPRT analysis based on age groups. In such cases, all the calculations are performed at a more granular level\DIFaddbegin \DIFadd{, }\DIFaddend and all the hyperparameters, such as the historical comparator rate, are taken based on age grouping. For example, age strata can be 5-year or 10-year age groups, or bespoke grouping according to vaccine schedule. More small-ranged grouping can also be performed to address the risk of different AESIs for different age ranges. Moreover, unlike the static binning detailed above, a dynamic binning approach of risk and AEFI-based age grouping is also possible where all groups may not have the same interval. This could provide an option of sequential analysis. This kind of analysis has not been tried yet in the literature, to the best of our knowledge.
...
After analysing the relevant literature, we developed a taxonomy of variations of \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis }\DIFaddend as shown in Figure~\ref{fig:variationMaxSPRT}.

The structure and organisation of the taxonomy depict the factors that establish the \DIFdelbegin \DIFdel{SPRT subtypes and }\DIFdelend implementation variations in a hierarchical manner. \DIFdelbegin \DIFdel{The variation starts with the availability of baseline information required to do the computation for surveillance. In some systems and situations, the expected number of events comes with associated uncertainty.There are various reasons behind the uncertainty that can add additional challenges to surveillance. To deal with uncertainty in the projected expected number of events, a conditional maximised sequential probability ratio test }\DIFdelend \DIFaddbegin \DIFadd{In Section~\ref{sec:methods}, we explained how the variation starts with ... 
}

%DIF >  The taxonomy outlines the hierarchical factors influencing implementation variation in sequential vaccine safety analysis. As explained in Section~\ref{sec:methods}, the difference between SPRT and MaxSPRT starts with their varying assumptions on projected event counts and relative risk. The choice between Poisson MaxSPRT (pMaxSPRT) for cohort-based analysis and Binomial MaxSPRT (bMaxSPRT) for self-controlled or case-based settings is then determined by analytical design.  Another way that pMaxSPRT differs from its conditional form (cMaxSPRT) is the availability of background data. Then, dynamic Type I and II error control is made possible by Silva et al.'s alpha spending functions, which add another level of variation.  Using linear, concave, or convex functions, these techniques optimise sample size (ESS) and anticipated time to signal (ETS) by varying alpha allocation over time.  Convex functions increase power and stability in the face of uncertainty, whereas concave functions encourage early detection. This structure captures both methodological distinctions and practical factors shaping implementation in diverse surveillance contexts.

\DIFadd{The taxonomy outlines the hierarchical factors influencing implementation variation in sequential vaccine safety analysis. As explained in Section~\ref{sec:methods}, SPRT differs from MaxSPRT due to assumptions on anticipated event counts and relative risk; Poisson MaxSPRT and Binomial MaxSPRT (bMaxSPRT) are the results of choosing between cohort-based and case-based analysis, respectively. The usage of either conditional }\DIFaddend (cMaxSPRT) \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{li2010conditional} }\hskip0pt%DIFAUXCMD
was proposed by Li et. al. , with a known baseline risk function no longer needed. Instead, it considers the unpredictability that comes from both the surveillance population and historical data}\DIFdelend \DIFaddbegin \DIFadd{or unconditional (pMaxSPRT) models is further determined by the availability and dependability of background data. Another layer of diversity is added by the choice to use alpha spending and the choice of suitable spending functions depending on the goal, such as optimising anticipated time to signal (ETS) or expected sample size (ESS). All of these factors are included in the taxonomy}\DIFaddend .
...
%DIF >  \added{SHOULD I ADD alpha, alpha spending function, Beta, statistical power, significance...}
...
SAEFVIC (a Surveillance of Adverse Events Following Vaccination in the Community\DIFdelbegin \DIFdel{data) }\DIFdelend \DIFaddbegin \DIFadd{) data}\DIFaddend ~\citep{clothier2017evaluation} is a set of timely passive surveillance data consisting of AEFI reports submitted by vaccinees, their relatives, GPs, nurses, or other healthcare professionals. It aligns to \DIFaddbegin \DIFadd{the }\DIFaddend minimum data for AEFI surveillance as per WHO~\citep{world2014global}. Each row of the data contains a report converted to a transactional database format\DIFaddbegin \DIFadd{, }\DIFaddend where each report is identified by a unique identifier named $Vaccinee\_ID$. Corresponding to each unique identifier, there are the age and sex information of the vaccinee, the time stamps of the dose administration, reporting time (when was it posted), and report submission. It also contains the time to onset expressed in the number of days for the adverse event\DIFaddbegin \DIFadd{, }\DIFaddend followed by the description of the event, reaction sign/symptom or clinical term of the event name, and vaccine brand\DIFaddbegin \DIFadd{, }\DIFaddend as well as the dose received. There are other data that are neither relevant nor required for MaxSPRT analysis, and we filtered them in the preprocessing step.

Like other real-world datasets, our data \DIFdelbegin \DIFdel{have missing values}\DIFdelend \DIFaddbegin \DIFadd{has missing values, }\DIFaddend too. This needs special attention before the data can be used for analysis. Data are collected through an online interactive form that is used by end-users (reporters). Hence, it is prone to errors like human errors that include misspellings and typographical errors. This implies that we need to use some mechanism to deal with unclean data.
...
Here\DIFdelbegin \DIFdel{we explain step by step the }\DIFdelend \DIFaddbegin \DIFadd{, we explain the step-by-step }\DIFaddend calculation of each measure, factor, and parameter required to calculate \DIFaddbegin \DIFadd{the }\DIFaddend $LLR$ (Log-likelihood Ratio)\DIFaddbegin \DIFadd{, }\DIFaddend which is used to detect a signal in MaxSPRT analysis. Throughout the paper, \DIFdelbegin \DIFdel{as a running worked example to demonstrate the calculations, we have chosen }\DIFdelend \DIFaddbegin \DIFadd{we use }\DIFaddend a sample SAEFVIC dataset for a particular vaccine and \DIFdelbegin \DIFdel{``Abdominal pain'' }\DIFdelend \DIFaddbegin \DIFadd{"Abdominal pain" }\DIFaddend as an AEFI \DIFaddbegin \DIFadd{as a running example to demonstrate the calculations}\DIFaddend .
...
The length of exposure is used to estimate the expected number of events in some variations of \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{sequential }\DIFaddend analyses. This is roughly the cumulative count of exposed time per vaccinee after a dose is administered and until the reaction is set on them in the person-time unit. Definition~\ref{OexposedPersonTime} (Appendix~\ref{sec:Terminology}, Terminology) expresses a way of capturing the overall length of exposure for all vaccinee under consideration for a MaxSPRT analysis when dose administration information is not available at all, or at least not during the analysis in real-time.
...
\DIFdelbegin \DIFdel{AESIs }\DIFdelend \DIFaddbegin \DIFadd{AEFIs }\DIFaddend are not reported in real-time for various reasons. Even after being reported, AEFI recording may also be delayed. This phenomenon is known as observation delay. Due to such delays, at the time of analysis, the amount of the observed data for a particular study week is likely to be less than the actual amount. This leads to an overestimation of the expected number of events, which imposes a potential risk of missing a signal. To address the issue, \DIFdelbegin \DIFdel{MaxSPRT has a feature of approximation }\DIFdelend \DIFaddbegin \DIFadd{we can adopt an approximation technique, }\DIFaddend and this is known as calculating the proportion of data complete (the ratio of the observed and expected amount of data). We defined the term in Definition~\ref{defn:proportionDataComplete}.
...
\DIFdelbegin \subsubsection{\DIFdel{Test Margin, $M$}}%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{subsubsection:testMargin}
%DIFDELCMD < %%%
\DIFdel{To the best of our knowledge, this is a value first used by CDC~\mbox{%DIFAUXCMD
\citep{FDAMaxSPRT2021} }\hskip0pt%DIFAUXCMD
for MaxSPRT analysis. It is particularly used to fine-tune the estimation of the expected number of events $\mu$ and used to emphasise a particular AEFI and vaccine/drug pair. Although, in our implementation, we facilitate specifying this value, by default we have used 1.0 for $M$. In particular, this can be a value close to 1 such as 1.01.
}\DIFdelend %DIF >  \subsubsection{\removed{Test Margin, $M$}}\label{subsubsection:testMargin}
%DIF >  \changed{To the best of our knowledge, this is a value first used by CDC - IS IT A RIGHT CLAIM THAT THEY USED IT FOR THE FIRST TIME}~\citep{FDAMaxSPRT2021} for MaxSPRT analysis. It is particularly used to fine-tune the estimation of the expected number of events $\mu$ and used to emphasise a particular AEFI and vaccine/drug pair. Although, in our implementation, we facilitate specifying this value, by default we have used 1.0 for $M$. In particular, this can be a value close to 1 such as 1.01.

\subsubsection{Estimating the Expected number of events}\label{subsubsec:EstimateExpectNumOfEvens}

The expected number of events is the number of events expected to be observed without rejecting the null hypothesis. Calculating \DIFdelbegin \DIFdel{this number is not straightforward}\DIFdelend \DIFaddbegin \DIFadd{expected number of events is crucial and can be achieved in various ways}\DIFaddend . It depends on the available information\DIFaddbegin \DIFadd{, objective and type of the sequential analysis being performed}\DIFaddend . There is a significant difference in calculating or estimating this value based on the available information. An approach to estimate this measure is proposed in~\citep{FDAMaxSPRT2021} and defined in Definition~\ref{defn:ApproximateExpCount} as ``Approximate Expected number of events'' in Appendix~\ref{sec:Terminology}. 

In passive surveillance or \DIFdelbegin \DIFdel{reposting }\DIFdelend \DIFaddbegin \DIFadd{reporting }\DIFaddend systems, such as SAEFVIC, we do not have dose administration information in real time. In such systems, only the numerator, and the report count are available. Therefore, in this paper and in our current implementation, we focused on $l_{stiwa}$ (observed exposed person-time, see Appendix~\ref{sec:Terminology}, Terminology) based estimation of the denominator. Once all the required metrics for calculating the expected number of events ($\mu_{s}$) are found, we compute the expected count using Definition~\ref{defn:ApproximateExpCount} (Appendix~\ref{sec:Terminology}, Terminology). While processing the data, another column is added to it to keep the weekly cumulative expected number of event counts that is going to be utilised later to compute $LLR$. 
...
\DIFdelbegin \DIFdel{In several variations of MaxSPRT, using }\DIFdelend \DIFaddbegin \DIFadd{Using }\DIFaddend this factor is optional \DIFdelbegin \DIFdel{, such as in \mbox{%DIFAUXCMD
\citep{greene2011near}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{for sequential analysis and can be ignored without the risk of impacting the outcome significantly. However, using this parameter ensures better approximation of the likelihood of a signal}\DIFaddend . In case we decide to use this factor, the calculation of the proportion of observed data can be performed in two ways. Firstly, similar to the existing works \citep{FDAMaxSPRT2021,greene2011near}, we can use historical data for the same AEFI and the same vaccine or \DIFaddbegin \DIFadd{a }\DIFaddend similar vaccine. Secondly, where no historical data is available, a special approach should be adopted to calculate the $p(i)$ using the concurrent data by estimating the proportion of later weeks of exposure from the earlier weeks of exposure.
...
We used cumulative expected number of events, $\mu_{s}$ (a snapshot of calculation of all components/measures of $\mu$ is shown in Table~\ref{table:ComputationOfDifferentParameterValuesOfMaxSPRT}) and $Y_{s}$ up to week $s$, to calculate the $LLR$ using the formula in Definition~\ref{defn:llr}. Either, the $LLR$ or $Y_{s}$ is used to detect signals by comparing against the critical value, $CV$. Note, there are two types of critical values: (i) based on $LLR$ of the corresponding week's cumulative expected number of events, we denote it as $CV_{LLR_{\mu_{s}}}$ and (ii) based on $Y$\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{silva2015continuous}}\hskip0pt%DIFAUXCMD
}\DIFaddend , the corresponding week's observed event count, we denote it as $CV_{Y_{s}}$. In other words, say in week $t$, if $LLR_{t} \geq CV_{LLR_{\mu_{t}}}$ or $Y_{t} \geq CV_{Y_{t}}$, then \DIFdelbegin \DIFdel{there is a signal }\DIFdelend \DIFaddbegin \DIFadd{$H_{0}$ can be rejected and a signal can be announced}\DIFaddend .

\begin{table}[!htb]
\centering
\caption{Demonstration of \DIFaddbeginFL \DIFaddFL{sequential }\DIFaddendFL signal detection \DIFdelbeginFL \DIFdelFL{through MaxSPRT analysis }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{using an alpha spending approach }\DIFaddendFL for Abdominal Pain (null hypothesis, $H_{0}$\DIFdelbeginFL \DIFdelFL{= }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{: }\DIFaddendFL no signal\DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{; a signal is indicated }\DIFaddendFL when $H_{0}$ is rejected \DIFdelbeginFL \DIFdelFL{, there is a signal}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{based on time-varying critical values}\DIFaddendFL )}
\begin{adjustbox}{width=0.5\textwidth}
...
Before we started the implementation of MaxSPRT, we faced a number of challenges. The first challenge was establishing a clear understanding of the procedure, terms, and underlying mathematics due to its many variations. \DIFaddbegin \DIFadd{-- NEED TO BE CAREFUL. }\DIFaddend The variations depend on the availability, nature, and properties of the data in hand. In addition, there is no hard and fast rule or specified model for the implementation. Organisations have different capabilities and limitations that direct implementers towards different variations, as explained in Section~\ref{sec:introduction}.

The \DIFdelbegin \DIFdel{challenges }\DIFdelend \DIFaddbegin \DIFadd{absence of thorough, detailed instructions for doing sequential analysis from the bottom up was one of the biggest obstacles }\DIFaddend we faced during implementation\DIFdelbegin \DIFdel{were mainly due to the lack of information in the literature.  Most papers described the analysis process from a high level, with limited detailed information about the implementation. Although some papers such as~\mbox{%DIFAUXCMD
\citep{FDAMaxSPRT2021,mesfin2019use} }\hskip0pt%DIFAUXCMD
explain in more detail, they are not fully reproducible}\DIFdelend \DIFaddbegin \DIFadd{.  A number of studies, including~\mbox{%DIFAUXCMD
\citep{FDAMaxSPRT2021, mesfin2019use, r2021exact}}\hskip0pt%DIFAUXCMD
, provide insightful information, however they mostly concentrate on methodological or high-level analytical issues.  There is still a dearth of thorough documentation on the actual implementation procedure, especially when it comes to people with little experience with epidemiology or vaccination.  This emphasises the necessity of repeatable procedures that connect statistical theory to practical implementation in vaccination safety monitoring}\DIFaddend . As an example, the definition of $p(s,t)$ is insufficiently defined to reliably reproduce (as discussed in Section~\ref{subsec:pstCalculation}). The data source and technique to calculate the ``proportion of data complete" that captures and addresses the observation delay in reporting reactions add several variations in MaxSPRT, and dealing with the calculation of this term was challenging.
...
Interestingly, the data source and technique to calculate the background rates add some variations and thus\DIFaddbegin \DIFadd{, }\DIFaddend additional challenges are introduced. Data properties and characteristics\DIFaddbegin \DIFadd{, }\DIFaddend such as the distribution of the data\DIFaddbegin \DIFadd{, }\DIFaddend also increase complexity in the implementation. One of the significant inherent challenges in any process involving data is missing values. In the case of MaxSPRT analysis, missing information in the data needs special consideration. We handled the missing data with great care, explained in Appendix~\ref{subsec:Preprocessing}.
...
Moreover, it is also challenging to validate or assess if the implementation of MaxSPRT was correctly performed; to the best of our knowledge, there is no common and publicly available data to be used as a benchmark and standard outcome to compare the results due to the privacy, security, and ethics policies of the data used in the \DIFdelbegin \DIFdel{literature}\DIFdelend \DIFaddbegin \DIFadd{domain of public health}\DIFaddend . Choosing and using the right version of \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis }\DIFaddend adds to the complexity. To deal with all of these challenges, we implemented the framework in a way that allows the end user to exhibit different variations of \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis }\DIFaddend for analysis using some arguments, and adjustment or changes to their values. We listed the available variations in Section~\ref{subsec:IncorporatingVariation}.
...
\DIFdelendFL \DIFaddbeginFL \begin{figure*}[!htb]
    ...
    % \setlength{\belowcaptionskip}{0pt}
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\linewidth]{Flowchart_MaxSPRT.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.9\linewidth]{Flowchart_MaxSPRT.pdf}
    \DIFaddendFL \caption{Flowchart of \DIFdelbeginFL \DIFdelFL{MaxSPRT}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MaxSPRT-based  Sequential Analysis}\DIFaddendFL }
...
MaxSPRT exhibits numerous variations that we explained in Section~\ref{sec:introduction}. In this section, we explain what variations are allowed in our current implementation. From the beginning, our focus was to make the implementation as flexible as possible so that the end user could utilise different variations of \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis }\DIFaddend by tuning the values of the relevant parameters.
...
The equation in the definition takes into account several parameters, such as $l_{stiwa}$, $p(s, t)$, $\alpha$, and $M$. Each of these parameters introduces different variations of \DIFdelbegin \DIFdel{MaxSPRT}\DIFdelend \DIFaddbegin \DIFadd{sequential analysis}\DIFaddend , as discussed in previous sections (such as Sections~\ref{sec:introduction} and Appendix~\ref{Implementation}), at least because of different approaches available and adapted by various research groups in their implementations of the parameters and MaxSPRT. Moreover, if we follow the approach of~\citep{greene2011near}, we can add more variations. From our point of view, the following variations can be applied, and we provide the flexibility to use all the variations in our current implementation.
...
Note that in \DIFdelbegin \DIFdel{Option~\ref{FDAop1} }\DIFdelend \DIFaddbegin \DIFadd{Options~\ref{FDAop1} to \ref{FDAop4}}\DIFaddend , we have not used the test margin\DIFaddbegin \DIFadd{, }\DIFaddend unlike CDC's original calculation~\citep{FDAMaxSPRT2021}. We can also vary the calculation by not using any age stratification for $\theta$ and thus for other parameters, as we did in Option~\ref{FDAop2} and Option~\ref{FDAop4}. We also omitted the use of $p(s, t)$ in Option~\ref{FDAop3} and Option~\ref{FDAop4}. More such variations could be introduced by taking the combination of all four parameters used in Definition~\ref{defn:ApproximateExpCount} (Appendix~\ref{sec:Terminology}, Terminology).

In our future work, we plan to investigate the performance of our MaxSPRT implementation and compare it with other statistical measures for vaccine safety surveillance. There\DIFaddbegin \DIFadd{, }\DIFaddend we plan to incorporate these variations in the performance analysis and describe our findings in detail following~\citep{greene2011near}.

\subsection{Flowchart}\label{subsec:pseudo_codes}
Figure~\ref{fig:flowchartMaxSPRT}, shows a flow diagram of \DIFdelbegin \DIFdel{the MaxSPRT analysis}\DIFdelend \DIFaddbegin \DIFadd{various sequential analyses}\DIFaddend . It describes the steps from taking input through to signal detection, including possible decision points to get a variation of \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis }\DIFaddend utilising the available information and selecting from possible options. \DIFdelbegin \DIFdel{Note that our current implementation is focused on the variations of MaxSPRT where baseline information is not known or insufficient. }\DIFdelend The diagram, however, shows a general flow of processes for most of the possible implementations and variations\DIFdelbegin \DIFdel{, including sufficient baseline information known}\DIFdelend . 
...
In the previous sections, we introduced the variations of \DIFdelbegin \DIFdel{MaxSPRT}\DIFdelend \DIFaddbegin \DIFadd{sequential analysis implementation}\DIFaddend , the reasons and the factors behind the variations, the challenges associated with particular variations, and how to deal with them. We also explain how we implemented it with maximum flexibility to offer as many variations as possible while minimising editing to \DIFaddbegin \DIFadd{the }\DIFaddend underlying code. Our current implementation, however, does not include as yet all possible variations of \DIFdelbegin \DIFdel{MaxSPRT}\DIFdelend \DIFaddbegin \DIFadd{sequential analysis}\DIFaddend , however, our Object-Oriented code can be easily extended with new variations, functionalities, and parameters without breaking any working code or module. This extensibility offers the opportunity to add new variations and devise more effective and adaptable signal detection.
...
\begin{enumerate}
    ...
    \item Based on the calculation of \DIFaddbegin \DIFadd{the }\DIFaddend length of exposure. Currently, we use observed exposed person time. This exposed person-time can be calculated in a different and perhaps \DIFdelbegin \DIFdel{a }\DIFdelend more reasonable way that adds more effectiveness in the calculation of the length of exposure and thus \DIFaddbegin \DIFadd{the }\DIFaddend expected number of events.
    \item CDC has not used risk window slicing in~\citep{FDAMaxSPRT2021} but Greene et al. used risk window slicing in~\citep{greene2011near}. Hence, the risk window slicing can add another variation in \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{the sequential }\DIFaddend analysis to the conditional MaxSPRT.
...
\begin{itemize}
    ...
    \item \textbf{Performance benchmarking:} We plan to compare the performance of different variations of MaxSPRT and different statistical surveillance methods and techniques, such as proportional reporting ratio (PRR)~\citep{clothier2019early}, Empirical Bayes~\citep{harpaz2013empirical}, CUSUM~\citep{musonda2008monitoring},  for safety signal detection. This requires some standards of comparison, and our initial plan is to apply all the variations of \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis }\DIFaddend and all other statistical methods under consideration on the same data sample. We can then compare \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis }\DIFaddend variations in their capacity to detect signals under particular circumstances. Among the methods that can detect signals, we can rank them based on the time required to detect a signal. We can also check if the generated signal is a false alarm or not, by checking the results among the methods. This develops another way of evaluating the capability of the methods in signal detection. We can then perform the analysis of effectiveness among the methods by calculating precision and recall.
    \item \textbf{Providing wider accessibility:} Difficulties faced in \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend implementation can occur due to challenges associated with the range of potential variations \DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend no definite answer to some critical issues; and a \DIFdelbegin \DIFdel{relative lack of established literature}\DIFdelend \DIFaddbegin \DIFadd{lack of top-to-bottom detailed illustration of use cases}\DIFaddend . Another set of challenges  was the verification of the correctness and soundness of the implementation. This motivated us to write this paper and further plan to develop an Application Programming Interface (API) that can be used by researchers to detect signals using their data. Using an open-access API can also assist in verifying implementation by comparing implementation results with the output and scores generated by our API on the same data.    
    \item \textbf{Adaptability and extensibility:} Our \DIFdelbegin \DIFdel{current implementation is only for data with Poisson distribution. The next step in our research is to integrate the facility to cope with the data having binomial distribution.
    }\DIFdelend \DIFaddbegin \DIFadd{framework is a generic framework that adapts to the variation in the structure and schema of data for analysis. Moreover, it has the flexibility to accommodate new methods and ways of calculating parameter values required for the sequential analysis.
    %DIF >  \changed{WRONG -- Our current implementation is only for data with Poisson distribution.} The next step in our research is to integrate the facility to cope with the data having binomial distribution. \added{SAEFVIC AND BINOMIAL ARE QUITE DIFFERENT AS PLACEBO CONTROLLED ARE NOT AVAILABLE. BUT WE DID EXTENDED OUR IMPLEMENTAION FOR POLAR AND VSHL. }
}\DIFaddend \end{itemize}

\section{Conclusions}\label{sec:conclusion}
MaxSPRT is a complex but powerful method for sequential \DIFaddbegin \DIFadd{group }\DIFaddend and continuous surveillance. It provides flexibility for implementing in different situations and conducting surveillance with various data limitations. However, its flexibility comes with implementation and operational trade-offs. The associated challenges can make it difficult to implement in some contexts or organisations, which can prevent benefits from being realised.
...
This paper explained the MaxSPRT surveillance method in a new, different, clear, and intuitive way\DIFaddbegin \DIFadd{, }\DIFaddend aiming to support its reproduction by providing mathematical and scientific notations and definitions of the relevant terms that mitigate the gaps between concept and reality. We included a flowchart of the overall implementation method for different variations of \DIFdelbegin \DIFdel{MaxSPRT}\DIFdelend \DIFaddbegin \DIFadd{sequential analysis}\DIFaddend ; and also added a hierarchy of the variations to demonstrate the evolution of the methods. 
...
We discussed further variations of \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis }\DIFaddend that may better capture a signal and our plan to extend the work, aiming to provide greater support to the vaccine safety surveillance community and to the public health surveillance community more broadly. We intend to further the work with the empirical outcomes of the variations of \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis }\DIFaddend and other surveillance techniques.
...
%DIF >  \added{NEW REFERENCES}
%DIF >  \citep{r2021exact,silva2020optimal,silva2018type,silva2015continuous,leite2016near,pocock1977group,gordon1983discrete,jennison1999group,o1979multiple}
\DIFaddbegin 

\DIFaddend % \bibliographystyle{spmpsci}
...
\DIFaddbegin \DIFadd{We refer the interested reader to \mbox{%DIFAUXCMD
\cite{silva2015continuous} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\cite{silva2019alpha} }\hskip0pt%DIFAUXCMD
for the definitions of ``group'' and ``continuous'' sequential analysis, concave and convex type alpha spending functions, ETS, and ESS. Some of the other related and important terms are defined here.
}

%DIF >  \begin{definition} \textbf{ETS:} ... ... ... .
%DIF >  \end{definition}

%DIF >  \begin{definition} \textbf{ESS:} ... ... ... .
%DIF >  \end{definition}
\DIFaddend 
...
%DIF >  \changed{AFTER THE OBSERVED EXPOSED PERSON TIME, IT SEEMS LIKE THE IMPLEMENTATION VARIATION WE ARE WORKING IS NOT USING HISTORICAL INFO FOR CALCULATING EXPECTED COUNT AND USING LSTIWA IN PERSON TIME UNIT FROM CONCURRENT DATA AND NOT DIRECTLY RELATED DENOMINATOR AND JUST A BACKGROUND RATE THETA. HENCE THE OBSERVED EXPOSED ALSO BEING CALCULATED IN PERSON TIME. SO NEED TO DISCUSS THIS IN PLACED WHERE NEEDED TO EXPLAIN THE VARIATIONS AND THE ONE WE HAVE EXPLAINED HERE.}
\DIFaddbegin 

\DIFaddend \begin{definition}\label{OexposedPersonTime} \textbf{Observed exposed person-time:}
...
Let $s$ be the study week of a MaxSPRT analysis and $LLR_{t}$ be the Log-likelihood ratio for any intermediate week $1 \leq t \leq s$. According to~\citep{kulldorff2011maximized}, the value of $LLR_{t}$ (or $\mu_{t}$ if the analyser chooses this over $LLR_{t}$) that makes the alternative hypothesis true is defined as a critical value and denoted as $CV$.
\end{definition}

\DIFaddbegin \DIFadd{Depending on whether an alpha spending plan is employed, there are two techniques to establish critical values (CVs) in sequential analysis.  The CV stays constant during the whole surveeillance period in the absence of an alpha spending plans, providing a steady threshold for signal identification.  Using an alpha spending plan, on the other hand, makes the CV dynamic, changing at every analysis point according to the amount of Type I error spent.  More flexible and effective monitoring is made possible by this method, which preserves overall Type I error control across several data analyses while allowing for earlier signal discovery.
}

%DIF >  \added{ALSO THIS IS INDICATED IN THE GRAPHS OF MAXSPRT AS PER ONE OF THE PAPER TOO WHERE TOP LEFT AND BOTTOM-RIGHT GRAPHS ARE USED TO SEE IF CV IS CROSSED IN LLR VALUE OR MAXSPRT VALUE}

\DIFaddend There are two ways of detecting a signal. Firstly, if the cumulative number of events observed at any time is beyond a prespecified number \DIFaddbegin \DIFadd{aka $ETS$}\DIFaddend , then there is no need to \DIFdelbegin \DIFdel{perform any MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{further continue the }\DIFaddend analysis and the null hypothesis can be rejected straightaway. Otherwise, at any week $t$, if the $LLR_{t}$ (or $\mu_{t}$) is beyond the corresponding critical value $CV$ expressed in likelihood (or, expressed in event count), then there is a signal, that is, $LLR_{t} \geq CV_{LLR_{t}}$, where $CV_{LLR_{t}}$ or $\mu_{t} \geq CV_{\mu_{t}}$, where $CV_{LLR_{t}}$ or $CV_{\mu_{t}}$ is the critical value for $\mu_{t}$ being the cumulative expected number of events for week $t$.

Kulldorff et. al. \citep{kulldorff2011maximized} proposed the technique, calculation, and uses of $CV$s for signal detection. \DIFdelbegin \DIFdel{They also }\DIFdelend \DIFaddbegin \DIFadd{Silva et al. }\DIFaddend developed an R \DIFdelbegin \DIFdel{sequential package}\DIFdelend \DIFaddbegin \DIFadd{Sequential package~\mbox{%DIFAUXCMD
\citep{rSequential} }\hskip0pt%DIFAUXCMD
}\DIFaddend with all functionality native to \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis }\DIFaddend calculation. If someone uses R, they do not need to reinvent the wheel for computing CV and other parameters required for MaxSPRT analysis. Moreover, in their paper\DIFaddbegin \DIFadd{, }\DIFaddend they have given $CV$s for up to 1000 expected number of events, for the reason that if someone wants to use a $CV$ for signal detection, they do not need to compute $CV$s again. \DIFaddbegin \DIFadd{The R Sequential package provides comprehensive functionality for sequential analysis, supporting multiple test statistic scales (e.g., cumulative event counts ($Y_{t}$), MaxSPRT, Pocock, O'Brien-Fleming, and Wang-Tsiatis). Key features include:
}\begin{itemize*}
    \item \DIFadd{Exact calculation of statistical power
    }\item \DIFadd{Calculation of expected time to signal
    }\item \DIFadd{Computation of expected sample sizes
    }\item \DIFadd{Sample size determination for sequential designs
    }\item \DIFadd{Alpha spending plan design and optimization
    }\item \DIFadd{One-tailed and two-tailed testing capabilities
    }\item \DIFadd{Functions for sequential analysis of Poisson data (Analyze.Poisson)
    }\item \DIFadd{Functions for sequential analysis of binary data (Analyze.Binomial)
    }\item \DIFadd{Performance evaluations for Poisson thresholds (Performance.Threshold.Poisson)
    }\item \DIFadd{Performance evaluations for binomial thresholds (Performance.Threshold.Binomial)
    }\item \DIFadd{Support for multiple weighted outcomes
    }\item \DIFadd{Construction of fixed-width and fixed-accuracy confidence intervals
    }\item \DIFadd{Visualization and monitoring of sequential test boundaries and results
}\end{itemize*}

\DIFadd{All these capabilities are seamlessly available irrespective of the chosen test statistic scale.
}\DIFaddend 
...
\multicolumn{1}{|c|}{\textbf{V1}}                       & 5                                & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c|}{\begin{minipage}{0.04\textwidth}
%DIFDELCMD <       \includegraphics[width=\textwidth]{inject.png}
%DIFDELCMD <     \end{minipage}}   %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c|}{\faSyringe}   \DIFaddendFL & \multicolumn{5}{c|}{\cellcolor{gray!25}$l_{sti(w=0)a}=5$}                                                                                                                                                             & \DIFaddbeginFL \faAmbulance             \DIFaddendFL & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             &              & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             &              \\ \hline
\multicolumn{1}{|c|}{\textbf{V2}}                       & 4                                & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & {\DIFdelbeginFL %DIFDELCMD < \begin{minipage}{0.04\textwidth}
%DIFDELCMD <       \includegraphics[width=\textwidth]{inject.png}
%DIFDELCMD <     \end{minipage}%%%
\DIFdelendFL \DIFaddbeginFL \faSyringe\DIFaddendFL }   & \multicolumn{4}{c|}{\cellcolor{gray!25}$l_{sti(w=0)a}=4$}                                                                                                                         & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c|}{}             %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c|}{\faAmbulance}             \DIFaddendFL & \multicolumn{1}{c|}{}             &              & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             &              \\ \hline
\multicolumn{1}{|c|}{\textbf{V3}}                       & 12                               & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c|}{\begin{minipage}{0.04\textwidth}
%DIFDELCMD <       \includegraphics[width=\textwidth]{inject.png}
%DIFDELCMD <     \end{minipage}}   %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c|}{\faSyringe}   \DIFaddendFL &      \cellcolor{gray!25} $l=1$       & \multicolumn{7}{c|}{\cellcolor{gray!25}$l_{sti(w=1)a}=7$}                                                                                                                                                                                                                & \multicolumn{4}{c|}{\cellcolor{gray!25}$l_{sti(w=2)a}=4$}                                                                                                                         & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c|}{}             %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c|}{\faAmbulance}             \DIFaddendFL & \multicolumn{1}{c|}{}             &              \\ \hline
\multicolumn{1}{|c|}{\textbf{V4}}                       & 22                               & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c|}{\begin{minipage}{0.04\textwidth}
%DIFDELCMD <       \includegraphics[width=\textwidth]{inject.png}
%DIFDELCMD <     \end{minipage}}   %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c|}{\faSyringe}   \DIFaddendFL & \multicolumn{3}{c|}{\cellcolor{gray!25}$l_{sti(w=0)a}=3$}                                                                & \multicolumn{7}{c|}{\cellcolor{gray!25}$l_{sti(w=1)a}=7$}                                                                                                                                                                                                                & \multicolumn{4}{c|}{\cellcolor{gray!25}$l_{sti(w=2)a}=4$}                                                                                                                         & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             &              \\ \hline
\multicolumn{1}{|c|}{\textbf{V5}}                       & 11                               & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c|}{\begin{minipage}{0.04\textwidth}
%DIFDELCMD <       \includegraphics[width=\textwidth]{inject.png}
%DIFDELCMD <     \end{minipage}}   %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c|}{\faSyringe}   \DIFaddendFL & \multicolumn{2}{c|}{\cellcolor{gray!25}$l_{sti(w=0)a}=2$}                            & \multicolumn{7}{c|}{\cellcolor{gray!25}$l_{sti(w=1)a}=7$}                                                                                                                                                                                                                & \multicolumn{2}{c|}{\cellcolor{gray!25}$l_{sti(w=2)a}=2$}                                                 & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c|}{}             %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c|}{\faAmbulance}             \DIFaddendFL & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             &              \\ \hline
\multicolumn{1}{|c|}{\textbf{V6}}                       & 10                               & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             &              & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c|}{\begin{minipage}{0.04\textwidth}
%DIFDELCMD <       \includegraphics[width=\textwidth]{inject.png}
%DIFDELCMD <     \end{minipage}}   %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c|}{\faSyringe}   \DIFaddendFL & \multicolumn{4}{c|}{\cellcolor{gray!25}$l_{sti(w=0)a}=4$}                                                                                                    & \multicolumn{6}{c|}{\cellcolor{gray!25}$l_{sti(w=1)a}=6$}                                                                                                                                                                                                 & \DIFaddbeginFL \faAmbulance             \DIFaddendFL \\ \hline
\multicolumn{1}{|c|}{\textbf{V7}}                       & 6                                & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             &              & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c|}{\begin{minipage}{0.04\textwidth}
%DIFDELCMD <       \includegraphics[width=\textwidth]{inject.png}
%DIFDELCMD <     \end{minipage}}   %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c|}{\faSyringe}   \DIFaddendFL &      \cellcolor{gray!25}  $l=1$      & \multicolumn{5}{c|}{\cellcolor{gray!25}$l_{sti(w=1)a}=5$}                                                                                                                                                             & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c|}{}             %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c|}{\faAmbulance}             \DIFaddendFL &              \\ \hline
\multicolumn{1}{|c|}{\textbf{V8}}                       & 10                               & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             &              & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             &              & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{}             & \DIFdelbeginFL %DIFDELCMD < \multicolumn{1}{c|}{\begin{minipage}{0.04\textwidth}
%DIFDELCMD <       \includegraphics[width=\textwidth]{inject.png}
%DIFDELCMD <     \end{minipage}}   %%%
\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c|}{\faSyringe}   \DIFaddendFL & \multicolumn{4}{c|}{\cellcolor{gray!25}$l_{sti(w=0)a}=4$}                                                                                                    \\ \hline
...
%DIF >  Beamer presentation requires \usepackage{colortbl} instead of \usepackage[table,xcdraw]{xcolor}
\DIFaddbegin \begin{table*}[!htb]
\centering
\caption{\DIFaddFL{Demonstration of event counts and person-time calculation for concurrent Data assuming 7 days of risk window.}}
\begin{adjustbox}{width=\textwidth}
...
\multirow{-2}{*}{\textbf{VID}}       & \multirow{-2}{*}{\textbf{TTO}}                    & \multicolumn{1}{c|}{\textbf{Sat}} & \multicolumn{1}{c|}{\textbf{Sun}} & \multicolumn{1}{c|}{\textbf{Mon}} & \multicolumn{1}{c|}{\textbf{Tue}} & \multicolumn{1}{c|}{\textbf{Wed}} & \multicolumn{1}{c|}{\textbf{Thu}} & \textbf{\DIFaddFL{Fri}}                & \multicolumn{1}{c|}{\textbf{Sat}}             & \multicolumn{1}{c|}{\textbf{Sun}} & \multicolumn{1}{c|}{\textbf{Mon}} & \multicolumn{1}{c|}{\textbf{Tue}}                & \multicolumn{1}{c|}{\textbf{Wed}}                & \multicolumn{1}{c|}{\textbf{Thu}}                & \textbf{\DIFaddFL{Fri}}                & \multicolumn{1}{c|}{\textbf{Sat}} & \multicolumn{1}{c|}{\textbf{Sun}} & \multicolumn{1}{c|}{\textbf{Mon}}                & \multicolumn{1}{c|}{\textbf{Tue}} & \multicolumn{1}{c|}{\textbf{Wed}}                & \multicolumn{1}{c|}{\textbf{Thu}}                & \textbf{\DIFaddFL{Fri}}                \\ \hline
\textbf{\DIFaddFL{V1}}                          & \textbf{\DIFaddFL{5}}                                        & \multicolumn{1}{c|}{\faSyringe}          & \multicolumn{5}{c|}{\cellcolor[HTML]{CAC6C6}}                                                                                                                                     & \cellcolor[HTML]{CAC6C6}\faAmbulance & \multicolumn{1}{c|}{\cellcolor[HTML]{CAC6C6}} & \multicolumn{4}{c|}{\cellcolor[HTML]{F2EBEB}}                                                                                                                               & \multicolumn{1}{c|}{\cellcolor[HTML]{F2EBEB}\faAmbulance} & \cellcolor[HTML]{F2EBEB}    & \multicolumn{7}{c|}{\cellcolor[HTML]{F2EBEB}}                                                                                                                                                                                                                                                    \\ \hline
\textbf{\DIFaddFL{V2}}                          & \textbf{\DIFaddFL{4}}                                        & \multicolumn{6}{c|}{}                                                                                                                                                                                                 & \faSyringe                         & \multicolumn{4}{c|}{\cellcolor[HTML]{CAC6C6}}                                                                                                                            & \multicolumn{1}{c|}{\cellcolor[HTML]{CAC6C6}\faAmbulance} & \multicolumn{2}{c|}{\cellcolor[HTML]{CAC6C6}}                                  & \multicolumn{5}{c|}{\cellcolor[HTML]{F2EBEB}}                                                                                                                                                                   & \multicolumn{1}{c|}{\cellcolor[HTML]{F2EBEB}\faAmbulance} & \cellcolor[HTML]{F2EBEB}    \\ \hline
\textbf{\DIFaddFL{V3}}                          & \textbf{\DIFaddFL{12}}                                       & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{\faAmbulance}          & \multicolumn{3}{c|}{}                                                                                     & \multicolumn{1}{c|}{\faSyringe}          & \cellcolor[HTML]{CAC6C6}    & \multicolumn{6}{c|}{\cellcolor[HTML]{CAC6C6}}                                                                                                                                                                                                                                  & \cellcolor[HTML]{F2EBEB}    & \multicolumn{4}{c|}{\cellcolor[HTML]{F2EBEB}}                                                                                                                & \multicolumn{1}{c|}{\cellcolor[HTML]{F2EBEB}\faAmbulance} & \multicolumn{2}{c|}{\cellcolor[HTML]{F2EBEB}}                                  \\ \hline
\textbf{\DIFaddFL{V4}}                          & \textbf{\DIFaddFL{22}}                                       & \multicolumn{3}{c|}{}                                                                                     & \multicolumn{1}{c|}{\faSyringe}          & \multicolumn{3}{c|}{\cellcolor[HTML]{CAC6C6}}                                                       & \multicolumn{4}{c|}{\cellcolor[HTML]{CAC6C6}}                                                                                                                            & \multicolumn{2}{c|}{\cellcolor[HTML]{F2EBEB}}                                                       & \cellcolor[HTML]{F2EBEB}\faAmbulance & \multicolumn{7}{c|}{\cellcolor[HTML]{F2EBEB}}                                                                                                                                                                                                                                                    \\ \hline
\textbf{\DIFaddFL{V5}}                          & \textbf{\DIFaddFL{11}}                                       & \multicolumn{1}{c|}{\faAmbulance}          & \multicolumn{3}{c|}{}                                                                                     & \multicolumn{1}{c|}{\faSyringe}          & \multicolumn{2}{c|}{\cellcolor[HTML]{CAC6C6}}                   & \multicolumn{3}{c|}{\cellcolor[HTML]{CAC6C6}}                                                                         & \multicolumn{1}{c|}{\cellcolor[HTML]{CAC6C6}\faAmbulance} & \multicolumn{1}{c|}{\cellcolor[HTML]{CAC6C6}}    & \multicolumn{2}{c|}{\cellcolor[HTML]{F2EBEB}}                                  & \multicolumn{2}{c|}{\cellcolor[HTML]{F2EBEB}}                         & \multicolumn{1}{c|}{\cellcolor[HTML]{F2EBEB}\faAmbulance} & \multicolumn{4}{c|}{\cellcolor[HTML]{F2EBEB}}                                                                                                                         \\ \hline
\textbf{\DIFaddFL{V6}}                          & \textbf{\DIFaddFL{10}}                                       & \multicolumn{7}{c|}{}                                                                                                                                                                                                                               & \multicolumn{2}{c|}{}                                                             & \multicolumn{1}{c|}{\faSyringe}          & \multicolumn{4}{c|}{\cellcolor[HTML]{CAC6C6}}                                                                                                                                        & \multicolumn{3}{c|}{\cellcolor[HTML]{CAC6C6}}                                                                            & \multicolumn{3}{c|}{\cellcolor[HTML]{F2EBEB}}                                                                                           & \cellcolor[HTML]{F2EBEB}\faAmbulance \\ \hline
\textbf{\DIFaddFL{V7}}                          & \textbf{\DIFaddFL{6}}                                        & \multicolumn{1}{c|}{}             & \multicolumn{1}{c|}{\faAmbulance}          & \multicolumn{5}{c|}{}                                                                                                                                                       & \multicolumn{5}{c|}{}                                                                                                                                                                                                       & \multicolumn{1}{c|}{\faSyringe}                         & \cellcolor[HTML]{CAC6C6}    & \multicolumn{5}{c|}{\cellcolor[HTML]{CAC6C6}}                                                                                                                                                                   & \multicolumn{1}{c|}{\cellcolor[HTML]{CAC6C6}\faAmbulance} & \cellcolor[HTML]{F2EBEB}    \\ \hline
\textbf{\DIFaddFL{V8}}                          & \textbf{\DIFaddFL{10}}                                       & \multicolumn{3}{c|}{}                                                                                     & \multicolumn{1}{c|}{\faAmbulance}          & \multicolumn{3}{c|}{}                                                                               & \multicolumn{4}{c|}{}                                                                                                                                                    & \multicolumn{1}{c|}{\faAmbulance}                         & \multicolumn{2}{c|}{}                                                          & \multicolumn{2}{c|}{}                                                 & \multicolumn{1}{c|}{\faSyringe}                         & \multicolumn{4}{c|}{\cellcolor[HTML]{CAC6C6}}                                                                                                                         \\ \hline \hline
...
%DIF >  \begin{table*}
%DIF >      \centering
%DIF >  \caption{Demostration of Observed counts for concurrent Data for the situation in Table~\ref{table:lstiwaDemonstration} assuming 7 days of risk window.}
%DIF >  \begin{adjustbox}{width=\textwidth}
%DIF >      \begin{tabular}{|c|c|c|c|c|c|c|}\hline
%DIF >           Week&  Unvaxed Events&  Risk Events&  Control Events&  Unvaxed Person-days&  Risk  
%DIF >  Person-days& Control
%DIF >  Person-days\\\hline
%DIF >           1&  0&  &  &  0+6+5+3+4+7+7+7&  11& 0\\\hline
%DIF >           2&  0&  &  &  0x5+2+5+7&  30& 7+\\ \hline
%DIF >           3&  0&  &  &  0x7+2&  25& 7+\\ \hline
%DIF >      \end{tabular}
%DIF >      \end{adjustbox}
%DIF >  \label{tab:concurrentCountDemo}
%DIF >  \end{table*}

\DIFaddend \subsection{Processing of SAEFVIC Data}\label{furtherProcessingData}
%summary%
...
\DIFdelbegin \DIFdel{Kulldorff et al~\mbox{%DIFAUXCMD
\citep{FDAMaxSPRT2021} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Silva et al.~\mbox{%DIFAUXCMD
\citep{r2021exact} }\hskip0pt%DIFAUXCMD
}\DIFaddend and Greene et al~\citep{greene2011near} recommended the use of R programming language as there is a package called ``Sequential" in $R$ developed especially for sequential analysis and contains all the required functionalities for \DIFdelbegin \DIFdel{MaxSPRT and other similar continuous and sequential analyses. 
However, our review of the literature and domain reveals that the main dependency of MaxSPRT implementation on R is to calculate critical values, $CV$ for corresponding $\mu$ with a proper $\alpha$ spending plan~\mbox{%DIFAUXCMD
\citep{silva2019alpha}}\hskip0pt%DIFAUXCMD
. This $\alpha$ spending plan will tune the threshold of $\alpha$ value automatically based on the situation to detect a signal as soon as possible with as minimum type-1 error as possible. However, Kulldorff et al describes in ~\mbox{%DIFAUXCMD
\citep{kulldorff2011maximized} }\hskip0pt%DIFAUXCMD
that if someone wants to use the table of $CV$s for a specific $\alpha$ (note, the table contains $CV$ for three different values of $\alpha$, namely, 0.01, 0.05, }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis. 
%DIF >  \changed{NOT FULLY TRUE THOUGH, RATHER R SEQUENTIAL IS NOT FOR DATA WRANGLING YOU COULD CLARIFY THIS -- However, our review of the literature and domain reveals that the main dependency of MaxSPRT implementation on R is to calculate critical values, $CV$ for corresponding $\mu$ with a proper $\alpha$ spending plan}~\citep{silva2019alpha}. 
The Sequential R package provides a powerful framework for implementing sequential analysis through defining essential design parameters such as Type I and II error rates, surveillance length, }\DIFaddend and \DIFdelbegin \DIFdel{0.1) and for $\mu$ of up to 1000, }\DIFdelend \DIFaddbegin \DIFadd{statistical power. It enables users to define a concave, convex, or linear alpha spending function that dynamically distributes the Type I error among several looks and automatically modifies decision thresholds in response to progress tracking. This adjustment reduces false positives while assisting in the earliest possible signal detection. In order to ensure effective surveillance design, the program additionally computes and optimises Expected Time to Signal (ETS) and Expected Sample Size (ESS). Combining these components improves overall statistical power in continuous or group sequential vaccine safety monitoring and allows for flexible adjustment of significance thresholds. However, Kulldorff \mbox{%DIFAUXCMD
\cite{kulldorff2011maximized}}\hskip0pt%DIFAUXCMD
, Silva \mbox{%DIFAUXCMD
\cite{silva2019alpha} }\hskip0pt%DIFAUXCMD
and Li \mbox{%DIFAUXCMD
\cite{li2010conditional} }\hskip0pt%DIFAUXCMD
provided recalculated values for specific parameters such as ETS, ESS and CV for different alpha and significant level that if someone wants to use those values }\DIFaddend they do not need to regenerate the \DIFdelbegin \DIFdel{$CV$s}\DIFdelend \DIFaddbegin \DIFadd{values to save time for analysis}\DIFaddend . 
...
Our choice of Object-oriented programming style also helps in implementing the different variations of \DIFdelbegin \DIFdel{MaxSPRT }\DIFdelend \DIFaddbegin \DIFadd{sequential analysis }\DIFaddend by adding more than one method for the same measure/metric like $P(s,t)$, $\theta$, and $l_{stiwa}$. Therefore, in the later stage especially during MaxSPRT analysis different variations of MaxSPRT can be designed by just choosing parameter values and more variations can be added without breaking any working and existing code/module.





\section{\DIFdelbegin \DIFdel{Demonstration of $l_{stiwa}$ calculation}%DIFDELCMD < \MBLOCKRIGHTBRACE\label{demo_lstiwa}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In Table~\ref{table:lstiwaDemonstration}, we provide a visual demonstration of the calculation of $l_{stiwa}$ and how it is calculated for each person. As an example, vaccinee $V_{1}$ got a dose on the 1$^{st}$ day of week-1. Their $TTO$ was 5 days. Hence, the person was exposed from day 2 to day 6 of week-1. Hence, $l_{stiwa}$ of the person is denoted as $l_{sti(w=0)a}$ and calculated as 5. Also, vaccinee $V_{5}$ got a dose on the 5th day of week-1. Their $TTO$ was 11 days. Hence, the person was exposed from day 6 of week-1 to day 2 of week-3.  Hence, the $l_{stiwa}$ of the person for weeks 1, 2, and 3 are denoted and calculated as $l_{sti(w=0)a}=2$, $l_{sti(w=1)a}=7$ }\DIFdelend \DIFaddbegin \DIFadd{Implementation }\DIFaddend and \DIFdelbegin \DIFdel{$l_{sti(w=2)a}=2$, respectively. At the end, the last row of the table shows the weekly $l_{stiwa}$.  }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Similar to the observed event count, we have added additional columns for $l_{stiwa}$ for each age group $a$, and for weekly $l_{stiwa}$ by adding all age-group based $l_{stiwa}$ for a particular week $t$. As an illustration, Table~\ref{table:LstiwaCalculation} shows the computed weekly $l_{stiwa}$ and age-strata based administration of $l_{stiwa}$}\DIFdelend \DIFaddbegin \DIFadd{code repository}}\label{Implementation_Code_Repo}
\DIFadd{In order to provide repeatable and expandable vaccination safety monitoring, we used the R Sequential package to create our sequential analysis architecture.  Numerous techniques are supported by the implementation, such as binomial-type sequential testing, Poisson-based alpha spending approaches, MaxSPRT, and cMaxSPRT.  Our method is intended to be schema-flexible and generalised, reducing rigid data dependencies and facilitating interoperability across various datasets and surveillance contexts}\DIFaddend .

 \DIFdelbegin \section{\DIFdel{Variations in $p(s, t)$ calculation}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{variationsPST}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{To calculate the ratio for calculating $p(s, t)$, we need to know the total number of reports for the week. In the literature, this ratio is obtained from historical data . On the other hand, it is quite challenging to get the ratio in case the denominator is not known.  Estimating the ratio without a proper or exact denominator needs an approximation technique. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{One of the approximation techniques would be to take the total reports found so far as a denominator to compute the ratio. However, this technique would not be sufficient to approximate the ratio properly. Because, as shown in Table~\ref{table:demonstrationOfDelayInReporting}, it is quite possible that we have more information about earlier weeks and less information regarding later weeks. However, the ratio indicates a wrong proportion of data complete, that is, more for later weeks and less for earlier weeks, unlike a real scenario.  In such cases, earlier week ratios will be more accurate than the later weeks. We can estimate the ratios of later weeks from the earlier weeks to deal with this challenge.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Our idea is to estimate a certain number of later weeks from the remaining weeks' $p(s,t)$. As an example,
    we can compute the last 50\% of weeks’ $p(s,t)$ from all previous weeks’ $p(s,t)$. There is scope to take lesser or greater than 50\%. We propose the following equation to calculate the proportion with a certain number of weeks' $p(s,t)$ to be estimated.}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}\DIFdel{p(s,t)=\frac{1}{\lceil\frac{s}{d}\rceil} \sum^{\lceil\frac{s}{d}\rceil}_{w=1}{p(s, t-w)}}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Here we estimate the $p(s,t)$ for week $t$ by taking the average of the $p(s,t')$ where $t'$ is the first half of the study weeks taking the division or partition number, $d$ = 2.  We can substitute this equation with some other more appropriate estimator,
    as well as we can tune the equation by changing the values of $d$. As an example, if $d=3$, the equation will take the average of the 1st one-third weeks' $p(s,t)$.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Table~\ref{table:DemonstrationOfDifferentPSTCalculations} shows the possible ways of calculating $p(s,t)$ for the delay distribution of the reports shown in Table~\ref{table:demonstrationOfDelayInReporting}. All five different approaches shown in the table for computing $p(s,t)$ do not comply with CDC’s conclusions that the earlier weeks will have more data complete than the later weeks, but they also have different logic and merits. To choose the best and most effective one, further empirical assessment is required.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{Parameters for MaxSPRT analysis}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{MaxSPRTParams}
%DIFDELCMD < %%%
\DIFdel{This section introduces some parameters required to perform MaxSPRT analysis for continuous surveillance using Sequential package~\mbox{%DIFAUXCMD
\citep{rSequential}}\hskip0pt%DIFAUXCMD
. Most of these parameters are explained in the manual~\mbox{%DIFAUXCMD
\citep{rSequential} }\hskip0pt%DIFAUXCMD
with example scenarios and values. These parameters play a very important role in the analysis. 
...
%DIFDELCMD < \noindent
%DIFDELCMD < %%%
\textbf{\DIFdel{Sample size:}} %DIFAUXCMD
\DIFdel{A non-zero upper limit of the expected number of events under the null hypothesis is referred to as Sample size. This value is needed to early terminate the analysis when the observed number of events outnumbers this value under the null hypothesis (i.e. , the null hypothesis is not rejected yet). There are various available methods in the package to estimate an optimised sample size that reduces the estimated time to signal as well as with different values of type-I error, relative risk, and statistical significance.}%DIFDELCMD < \\
%DIFDELCMD < 

%DIFDELCMD < \noindent 
%DIFDELCMD < %%%
\textbf{\DIFdel{Statistical significance level ($\alpha$) and alpha spending plan:}} %DIFAUXCMD
\DIFdel{Statistical significance assesses whether the observed risk is not occurring due to random variation. The significance level $\alpha$ focuses on controlling Type-I errors and influences statistical significance in the analysis. MaxSPRT allows the defining or controlling of the alpha spending plan by the analyser. There are parameterisable methods to conduct an analysis using MaxSPRT where a specific plan to spend $\alpha$, such as }\DIFdelend \DIFaddbegin \DIFadd{Table X (see Figure) provides a summary of the SAEFVIC database design, which serves as the basis for the underlying data structure used for the sequential analysis.  Our implementation only needs a limited number of fields to work properly}\DIFaddend , \DIFdelbegin \DIFdel{uniform in every analysis period, or conservative or generous at the beginning and changing exponentially or logarithmically in spending $\alpha$) can be executed. As per the plan, a certain amount of alpha will be spent in each analysis period (where period means weeks if the data is stratified into weeks and analysed weekly). $\rho$ is a positive number used for the power type alpha spending function.}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{even if the whole schema includes extensive metadata such as vaccine identities, vaccination and reporting timestamps, demographic information, and thorough reaction descriptions.  To be more precise, any dataset that contains the following fields is adequate for doing reliable sequential analysis:
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \noindent
%DIFDELCMD < %%%
\textbf{\DIFdel{Statistical Power:}} %DIFAUXCMD
\DIFdel{Statistical power measures the probability of identifying the risk if it is truly present. It focuses on controlling Type-II errors and is influenced by sample size and significance level.
}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{enumerate*}[label=(\roman*)]
    \item \DIFadd{`VacID' (unique report identifier),
    }\item \DIFadd{either `Time of Vaccination' or a comparable timestamp (e.g., `Vaxdate'),
    }\item \DIFadd{a reaction event date (from which Time to Onset (TTO) can be derived), and
    }\item \DIFadd{`Reactions' (the event or condition of interest).
}\end{enumerate*}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \noindent
%DIFDELCMD < %%%
\textbf{\DIFdel{Relative risk:}} 
%DIFAUXCMD
\DIFdel{The relative risk $RR$ is the ratio of incidence rate in exposed and unexposed populations. In other words, this is the ratio of observed and expected rates of events. It is required that $RR > 1$, because, with $RR = 1$ means there is no difference between observed and expected incidences and hence no risk due to the vaccine. The higher the value of $RR$ the more the risk is and the smaller will be the sample size if statistical significance and alpha remain constant. }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Because of this generalisation, the system can be used in a variety of ways, including when complete clinical or demographic data is not available, as is frequently the situation in post-market vaccine surveillance across jurisdictions. The code is modular, enabling customisation of event definitions, stratification, and filtering to meet various surveillance objectives. It is also flexible with respect to input formats—for instance, gender can be represented as Male/Female/Others, 0/1/2, or M/F/O, and if Date of Birth is provided instead of age, the system adjusts calculations accordingly.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \noindent
%DIFDELCMD < %%%
\textbf{\DIFdel{Minimum number of events:}} 
%DIFAUXCMD
\DIFdel{In vaccine safety surveillance, normally a minimum number of events need to be observed before rejecting the null hypothesis and marking the vaccine unsafe. This minimum number of events must be positive and should be 4~\mbox{%DIFAUXCMD
\citep{kulldorff2017continuous}}\hskip0pt%DIFAUXCMD
.}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Interested people can get the whole implementation in our public GitHub repository, which includes visual and textual output generation code, parameter configurations, and preprocessing scripts:
